...
import { distinctUntilKeyChanged, filter, switchMap, take, takeUntil, tap } from 'rxjs/operators';
import { INgxFileUploadRequest, NgxFileUploadState, NgxFileUploadStorageConfig } from '../../api';
...
export class NgxFileUploadStorage {
  ...
  add(upload: INgxFileUploadRequest | INgxFileUploadRequest[]) {
    ...
    requests.forEach((request: INgxFileUploadRequest) => {
      ...
      // @todo better interface we only want run method but we do not want publish it
      // better strategy for this ?
      this.registerUploadEvents(request as NgxFileUploadRequest);
    });
  ...
  private queueRequest(request: NgxFileUploadRequest) {
